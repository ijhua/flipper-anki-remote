...
enum HidDebugSubmenuIndex {
    HidSubmenuIndexAnkiRemote,
};

static void hid_submenu_callback(void* context, uint32_t index) {
    furi_assert(context);
    Hid* app = context;
    if(index == HidSubmenuIndexAnkiRemote) {
        app->view_id = HidViewAnkiRemote;
        view_dispatcher_switch_to_view(app->view_dispatcher, HidViewAnkiRemote);
    }
}
...
static void bt_hid_connection_status_changed_callback(BtStatus status, void* context) {
    ...
    hid_anki_remote_set_connected_status(hid->hid_anki_remote, connected);
}
...
Hid* hid_alloc(HidTransport transport) {
    ...
    submenu_add_item(
        app->device_type_submenu, "Anki Remote", HidSubmenuIndexAnkiRemote, hid_submenu_callback, app);
...
Hid* hid_app_alloc_view(void* context) {
    ...
    // Anki view
    app->hid_anki_remote = hid_anki_remote_alloc(app);
    view_set_previous_callback(hid_anki_remote_get_view(app->hid_anki_remote), hid_exit_confirm_view);
    view_dispatcher_add_view(
        app->view_dispatcher, HidViewAnkiRemote, hid_anki_remote_get_view(app->hid_anki_remote));
...
void hid_free(Hid* app) {
    ...
    // Free views
    view_dispatcher_remove_view(app->view_dispatcher, HidViewAnkiRemote);
    hid_anki_remote_free(app->hid_anki_remote);
